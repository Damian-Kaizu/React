    ...
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
        ...
        <div className="flex-[1.5] w-full flex flex-row justify-between flex-warp md:mt-0 mt-10 ">
          {footerLinks.map((footerLink) => (
            <div key={footerLink.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
              <h4 className={`font-poppins font-normal text-[18px] leading-[27px] text-white`}>{footerLink.title}</h4>
     ...
     <div className="flex flex-row md:mt-0 mt-60">
                  {socialMedia.map((social,index)=>(
                    <img key={social.id} 
                    src={social.icon}
                    alt="icon" className={`w-[21px] h-[21px] object-contain cursor-pointer ${index !== socialMedia.length -1 ? "mr-6" : "mr-0"}`}/>
                  ))}
     </div>
      </div>
    </section>
